...
console.log("leveldb: " + config.leveldb);

var db = imports.db || levelup(leveldown(config.leveldb + '/txs', {
...
var encodedData = require('soop').load('marscore/lib/util/encodeddata', {
  base58: base58
});
var versionedData = require('soop').load('marscore/lib/util/versioneddata', {
  parent: encodedData
});

var Address = require('soop').load('marscore/lib/address', {
...
TransactionDb.prototype.getStandardizedTx = function(tx, time, isCoinBase) {
  ...
  tx.vin = tx.ins.map(function(txin) {
    ...
    if (isCoinBase) {
      ...
      ret.txid = new Buffer(txin.getOutpointHash()).reverse().toString('hex');
...
TransactionDb.prototype.removeFromTxId = function(txid, cb) {
  async.series([
      ...
      function(c) {
        db.createReadStream({
          gte: OUTS_PREFIX + txid + '-',
          lte: OUTS_PREFIX + txid + '~',
        }).pipe(
      ...
      function(c) {
        db.createReadStream({
          gte: SPENT_PREFIX + txid + '-',
          lte: SPENT_PREFIX + txid + '~'
        })
          .pipe(
            db.createWriteStream({
              type: 'del'
