...
// Reverse the buffer using Buffer methods
for (let i = 0, j = genesisHashReversed.length - 1; i < j; i++, j--) {
    const temp = genesisHashReversed[i];
    genesisHashReversed[i] = genesisHashReversed[j];
    genesisHashReversed[j] = temp;
}
